        ...
        {step === 2 && (
          <>
            <div className="flex flex-col items-start gap-2">
              ...
              <h1 className="text-xl md:text-2xl font-bold text-gray-700">Script Details</h1>
            </div>
            <div className="flex gap-4">
              <label className="flex-1">
                <span className="font-medium text-gray-700 mb-1 block">Tone</span>
                <select className="w-full p-2 border rounded" value={tone} onChange={e => setTone(e.target.value)} required>
        ...
        {step === 3 && (
          <>
            ...
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mt-4 rounded">
              <h3 className="font-bold text-yellow-700 mb-1">Pro Tips for This Script:</h3>
              <ul className="list-disc ml-5 text-sm text-yellow-800">
                <li>Keep your hook under 3 seconds for maximum retention.</li>
                <li>Use short, energetic sentences and punchy visuals.</li>
                <li>Always finish with a strong call to action (CTA).</li>
                <li>Show, don’t tell: use quick demos or success scenes.</li>
              </ul>
            </div>
            <div className="flex justify-between items-center gap-4 mt-8">
              <button type="button" className="px-6 py-2 bg-gray-100 text-gray-700 rounded-lg font-semibold shadow hover:bg-gray-200 transition"
                onClick={() => { setStep(2); setScript(""); }}>Previous</button>
              <button type="button" className="bg-gray-300 px-6 py-2 rounded-lg font-bold text-gray-700 shadow hover:bg-gray-400 transition mx-auto"
                onClick={() => navigator.clipboard.writeText(script)}>Export</button>
              <button type="button" className="bg-[#1db5be] text-white px-6 py-2 rounded-lg font-bold shadow hover:bg-[#13a0ae] transition"
                onClick={() => { setStep(1); setObjective(''); setTone(''); setProductInfo(''); setScript(''); }}>Create Another</button>
            </div>
          </>
        )}
      </form>
